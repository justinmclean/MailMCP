...
from apache_incubator_mail_mcp import tools
    ...
    if method == "initialize":
        return jsonrpc_result(
            message_id,
            {
                ...
                "serverInfo": {"name": "apache-incubator-mail-mcp", "version": "0.1.0"},
                "capabilities": {"tools": {}},
            },
        )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Apache Incubator Mail MCP server")
    parser.add_argument("--api-base", help="Pony Mail API base URL")
    parser.add_argument("--cache-dir", help="Path to cache Incubator general mail summaries")
    return parser.parse_args(argv)


def main(argv: list[str] | None = None) -> int:
    args = parse_args(argv)
    tools.configure_defaults(api_base=args.api_base, cache_dir=args.cache_dir)
